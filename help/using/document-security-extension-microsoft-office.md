---
title: Microsoft用AEM Document Security拡張機能の概要（&R）
description: Document Security Extension for Microsoft&reg; Officeを使用すると、定義済みの機密性設定をMicrosoft&reg; Office ファイルに適用できます。
...
* 管理者またはポリシーの適用者がファイルへのアクセスを無効にするか、ポリシーを変更するまで。
  ...
  ポリシーの適用者がポリシーを変更するかファイルへのアクセスを無効にした場合、ユーザーが既にファイルを持っていても、そのファイルに対するユーザーの権限が変更または削除されます。 ファイル自体が取り消された場合は、更新されたコピーを取得するためのURLをユーザーに提供できます。
...
次の表に、AEM Document Security の参考リソースを記載します。
